...
from torch.utils.data import DataLoader, Dataset # 导入Pytorch的 DateLoader 和 Dataset

tokenizer = get_tokenizer('basic_english') # 定义数据预处理所需的Tokenizer
train_iter = read_file('../../01_Data/wikitext-103/wiki.train02.txt')# 加载Wikitext2数据集的训练集
valid_iter = read_file('../../01_Data/wikitext-103/wiki.valid.txt')# 加载Wikitext2数据集的验证集
...
# 第2步 构建Pytorch数据集
from torch.utils.data import Dataset # 导入Dataset 类
...
class WikiDataset(Dataset):
    def __init__(self, data_iter, vocab, max_len=max_seq_len):
        ...
        for sentence in data_iter: # 遍历数据集，将文本转换为tokens
            ...
            self.data.append(tokens) # 将处理好的tokens 添加到数据集中
...
# 第3步 构建DataLoader类
# 定义pad_sequence 函数，用于将一批序列补齐到相同长度
...
# 定义 collate_fn 函数，用于将一个批次的数据整理成适当的形状
...
# 创建一个训练数据加载器，用于自定义的collate_fn函数
...
for epoch in range(epochs):
    ...
    # 训练代码
    for batch_idx, (source, target) in enumerate(train_dataloader): # 用 dataloader 加载数据
