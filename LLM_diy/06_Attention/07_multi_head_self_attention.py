...
class Attention(nn.Module):
    ...
    #     assert decoder_context.size(-1) == num_heads * head_dim
    #
    #     # 定义线性层用于将 x 转换为 Q, K, V 向量
    #     linear_q = torch.nn.Linear(128, 128)
    #     linear_k = torch.nn.Linear(128, 128)
    #     linear_v = torch.nn.Linear(128, 128)
    #
    #     # 通过线性层计算 Q, K, V
    #     Q = linear_q(decoder_context)  # (batch_size=2, seq_len=3, Q_dim=128)
    #     K = linear_k(encoder_context)  # (batch_size=2, seq_len=3, K_dim=128)
    #     V = linear_v(encoder_context)  # (batch_size=2, seq_len=3, V_dim=128)
    #
    ...
    #
    #     Q = split_heads(Q, num_heads)  # (batch_size=2, num_heads=2, seq_len=3, head_dim=64)
    #     K = split_heads(K, num_heads)  # (batch_size=2, num_heads=2, seq_len=3, head_dim=64)
    #     V = split_heads(V, num_heads)  # (batch_size=2, num_heads=2, seq_len=3, head_dim=64)
    #
    ...
    #
    #     # 对拼接后的结果进行线性变换
    #     linear_out = torch.nn.Linear(128, 128)
    #     attn_outputs = linear_out(attn_outputs)  # (batch_size=2, seq_len=3, output_dim=128)
    #     print("加权信息:", attn_outputs)
    #
    #     # 将注意力权重乘以encoder_context，得到加权的上下文向量
    #     context = torch.matmul(attn_outputs, encoder_context)
    #     return context, attn_outputs

    def forward(self, decoder_context):
        # q = decoder_context
        # k = decoder_context
        # v = decoder_context
        ...
        # 通过线性层计算 Q, K, V
        Q = linear_q(decoder_context)  # (batch_size=2, seq_len=3, Q_dim=128)
        K = linear_k(decoder_context)  # (batch_size=2, seq_len=3, K_dim=128)
        V = linear_v(decoder_context)  # (batch_size=2, seq_len=3, V_dim=128)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, inputs, hidden, encoder_outputs):
        ...
        context = self.attention(rnn_output)  # 计算注意力上下文向量
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, hidden, target):
        ...
        decoder_output, _ = self.decoder(target, decoder_hidden, encoder_output)
...
def visualize_attention(source_sentence, predicted_sentence, attn_weights):
    ...
    ax = sns.heatmap(attn_weights, annot=True, cbar=False, xticklabels=source_sentence.split(),
