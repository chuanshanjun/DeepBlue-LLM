...
# 自回归模型在训练及生成的不同过程有不同作用
# 在生成过程，首先，我们提供一个初始的种子文本（即提示或指令），然后模型会根据这个种子文本生成下一个词。
# 生成的词将被添加到文本中，继续输入模型，模型会接着生成下一个词

# 定义贪婪解码器函数
def greedy_decoder(model, enc_input, start_symbol):
    # 对输入数据进行编码，并获得编码器输出及自注意力权重
    enc_output, enc_self_attns = model.encoder(enc_input)
    # 初始化编码器输入张量全为零，大小为(1,5)，数据类型与enc_input一致
