...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs):
        # 创建位置信息
        position = torch.arange(len(dec_inputs), device=dec_inputs.device).unsqueeze(-1)
        # 将词嵌入与位置编码相加
        inputs_embedding = self.src_emb(dec_inputs) + self.pos_emb(position)
        # 生成自注意力掩码，注意要将类型从uint8转换成bool类型
...
class LanguageCorpus:
    def __init__(self, sentences):
        ...
        # 计算语言的最大句子长度，并加2义容纳特殊符号<sos>合<eos>
...
# 我们根据文件lang.txt创建一个语料库实例，并显示其中的一些信息
with open('./data/lang.txt', 'r') as file: # 从文件中读入语料
    sentences = [line.strip() for line in file.readlines()]
    corpus = LanguageCorpus(sentences) # 创语料库
    vocab_size = len(corpus.vocab) # 词汇表大小
    max_seq_len = corpus.seq_len # 最大句子长度(用于设置编码)
...
epochs = 500 # 训练轮次
for epoch in range(epochs): # 训练epochs 轮
